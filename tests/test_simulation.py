...
def test_droop_response() -> bool:
    """
    Verify FleetModel.apply_droop_response() applies governor droop to all
    ONLINE synchronous units (including non-AGC-eligible fuel types), and
    verify _AGC_UNIT_TYPES no longer includes HYDRO_ROR/HYDRO_PUMP.

    Checks:
      - A frequency deficit (Δf < 0) raises output on COAL, NUCLEAR, CCGT,
        HYDRO, HYDRO_ROR, and HYDRO_PUMP units alike (droop is universal).
      - WIND and SOLAR output is untouched by droop.
      - Response direction opposes the deviation (deficit -> raise).
      - apply_agc_signal()/agc_regulation_state() only ever move/report
        HYDRO and CCGT units — HYDRO_ROR and HYDRO_PUMP are excluded.
    """
    print("test_droop_response...")
    all_passed = True

    try:
        from simulation.units import FleetModel, _AGC_UNIT_TYPES
        from simulation.grid import Grid

        # ── AGC eligibility set is exactly {HYDRO, CCGT} ──────────────────
        try:
            assert _AGC_UNIT_TYPES == frozenset({'HYDRO', 'CCGT'}), \
                f"_AGC_UNIT_TYPES should be {{'HYDRO', 'CCGT'}}, got {_AGC_UNIT_TYPES!r}"
            print(f"  _AGC_UNIT_TYPES = {sorted(_AGC_UNIT_TYPES)} — PASS")
        except AssertionError as e:
            print(f"  AGC eligibility set: FAIL — {e}")
            all_passed = False

        # ── Droop raises output on every synchronous fuel type ────────────
        try:
            g7 = Grid(7)  # Shift 7: COAL, NUCLEAR, CCGT, HYDRO, HYDRO_ROR,
                          # HYDRO_PUMP, WIND, SOLAR all active
            schedule = {
                u.label: max(u.min_mw, u.rated_mw * 0.5)
                for u in g7.get_active_units()
                if u.unit_type not in ('WIND', 'SOLAR')
            }
            fleet = FleetModel(g7, initial_schedule=schedule)

            pre_mw = {
                label: m.current_mw for label, m in fleet._units.items()
                if m.state == 'ONLINE'
            }

            # 0.2 Hz deficit -> droop should raise every online synchronous unit.
            assignments = fleet.apply_droop_response(delta_f_hz=-0.2)
            assert assignments, "Droop should return non-empty assignments for a 0.2 Hz deficit"

            checked_types: set[str] = set()
            for label, delta in assignments.items():
                model = fleet._units[label]
                assert delta > 0.0, \
                    f"{label}: deficit should raise output (droop delta > 0), got {delta:.3f}"
                assert model.current_mw > pre_mw[label], \
                    f"{label}: current_mw should increase after droop, " \
                    f"{pre_mw[label]:.1f} -> {model.current_mw:.1f}"
                checked_types.add(model._spec.unit_type)

            required_types = {'COAL', 'NUCLEAR', 'CCGT', 'HYDRO', 'HYDRO_ROR', 'HYDRO_PUMP'}
            missing = required_types - checked_types
            assert not missing, \
                f"Droop should touch all synchronous types; missing from response: {missing}"

            print(f"  Deficit raises output on all synchronous types "
                  f"({sorted(checked_types)}) — PASS")
        except AssertionError as e:
            print(f"  Universal droop response: FAIL — {e}")
            all_passed = False

        # ── WIND/SOLAR untouched by droop ──────────────────────────────────
        try:
            g7 = Grid(7)
            fleet2 = FleetModel(g7, initial_schedule={})
            renewable_pre = {
                m.label: m.current_mw for m in fleet2._units.values()
                if m.is_renewable
            }
            fleet2.apply_droop_response(delta_f_hz=-0.5)
            for label, mw_before in renewable_pre.items():
                model = fleet2._units[label]
                assert model.current_mw == mw_before, \
                    f"{label} (renewable) should be unaffected by droop, " \
                    f"{mw_before:.1f} -> {model.current_mw:.1f}"
            print(f"  WIND/SOLAR unaffected by droop ({len(renewable_pre)} units checked) — PASS")
        except AssertionError as e:
            print(f"  Renewable exclusion: FAIL — {e}")
            all_passed = False

        # ── Surplus lowers output (opposite direction) ─────────────────────
        try:
            g7 = Grid(7)
            schedule = {
                u.label: u.rated_mw
            ...
            fleet3 = FleetModel(g7, initial_schedule=schedule)
            assignments = fleet3.apply_droop_response(delta_f_hz=0.2)
            assert assignments, "Droop should return assignments for a 0.2 Hz surplus"
            for label, delta in assignments.items():
                assert delta < 0.0, \
                    f"{label}: surplus should lower output (droop delta < 0), got {delta:.3f}"
            print(f"  Surplus lowers output (opposes deviation) — PASS")
        except AssertionError as e:
            print(f"  Surplus direction: FAIL — {e}")
            all_passed = False

        # ── apply_agc_signal only ever moves HYDRO/CCGT ────────────────────
        try:
            g7 = Grid(7)
            schedule = {
                u.label: max(u.min_mw, u.rated_mw * 0.5)
            ...
            fleet4 = FleetModel(g7, initial_schedule=schedule)
            assignments = fleet4.apply_agc_signal(delta_mw=50.0)
            assert assignments, "AGC should return non-empty assignments"
            for label in assignments:
                unit_type = fleet4._units[label]._spec.unit_type
                assert unit_type in ('HYDRO', 'CCGT'), \
                    f"{label} ({unit_type}) should not receive AGC dispatch"

            current, min_mw, max_mw = fleet4.agc_regulation_state()
            assert max_mw > 0.0, "agc_regulation_state() max_mw should be > 0 with HYDRO/CCGT online"
            print(f"  AGC only dispatches HYDRO/CCGT "
                  f"(regulation range {min_mw:.0f}-{max_mw:.0f} MW) — PASS")
        except AssertionError as e:
            print(f"  AGC fuel-type restriction: FAIL — {e}")
            all_passed = False
